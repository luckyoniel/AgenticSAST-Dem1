...
$rawOutput | Out-File -FilePath "$PSScriptRoot\sse-glm-output.txt" -Encoding UTF8
Write-Host "Raw output saved to sse-glm-output.txt ($totalReceived bytes)"
Write-Host "---"
    ...
    if ($line.StartsWith("data:")) {
        ...
        try {
            ...
            if ($parsed.PSObject.Properties.Name -contains "isError" -and $parsed.isError -eq $true) {
                ...
                if ($eventCount -le 5) {
                    $preview = if ($c.Length -gt 100) { $c.Substring(0, 100) } else { $c }
